...
1. Descargue las imágenes de entrenamiento de `https://github.com/MicrosoftLearning/mslearn-ai-vision/raw/main/Labfiles/03-object-detection/training-images.zip` y extraiga la carpeta comprimida para ver su contenido. Esta carpeta contiene imágenes de fruta.
...
1. De nuevo en Azure Portal, ejecute el comando `cd C-Sharp/train-detector` o `cd Python/train-detector` en función de sus preferencias de lenguaje.
...
1. Con el comando `ls`, puede ver el contenido de la carpeta **train-detector**. Observe que contiene un archivo para los valores de configuración:
...
1. Tenga en cuenta que la carpeta **train-detector** contiene una subcarpeta en la que se almacenan los archivos de imagen a los que se hace referencia en el archivo JSON.

1. Tenga en cuenta que la carpeta **train-detector** contiene un archivo de código para la aplicación cliente:
...
1. Ejecute el siguiente comando para ejecutar el programa:
...
1. Espere a que se complete el entrenamiento (puede tardar alrededor de 10 minutos) y compruebe las métricas de rendimiento *Precision*, *Recall* y *mAP* (miden la precisión de la predicción del modelo de clasificación y sus valores deberían ser altos).
...
1. En Azure Portal, vaya a la carpeta **test-detector** ejecutando el comando `cd ../test-detector`.
1. Escriba el siguiente comando específico del SDK para instalar el paquete de predicción de Custom Vision:
...
1. Una vez completado el programa, en la barra de herramientas de Cloud Shell, seleccione **Cargar o descargar archivos** y, a continuación, elija **Descargar**. En el cuadro de diálogo nuevo, escriba la siguiente ruta de acceso del archivo y seleccione **Descargar**:
...
1. Examine el archivo **output.jpg** resultante para ver los objetos detectados en la imagen.
